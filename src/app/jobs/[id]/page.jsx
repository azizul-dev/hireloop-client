...
import { LocationArrowFill, Briefcase, Globe, Tag } from "@gravity-ui/icons";
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-[#121214] border border-neutral-800 rounded-2xl p-6 flex flex-col sm:flex-row gap-5 items-start sm:items-center justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="flex items-center gap-1 text-xs px-3 py-1 rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/20">
            ...
            <span className="flex items-center gap-1 text-xs px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
              <Globe className="w-3 h-3" /> {isRemote ? "Remote" : "On-site"}
            </span>
            <span className="flex items-center gap-1 text-xs px-3 py-1 rounded-full bg-neutral-800 text-neutral-400 border border-neutral-700">
              <Tag className="w-3 h-3" /> {category}
            </span>
            <span className="flex items-center gap-1 text-xs px-3 py-1 rounded-full bg-neutral-800 text-neutral-400 border border-neutral-700">
              <LocationArrowFill className="w-3 h-3" /> {location}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            {
              label: "Deadline",
              value: new Date(deadline).toLocaleDateString("en-GB"),
            },
            {
              label: "Status",
            ...
            <div
              key={item.label}
              className="bg-[#121214] border border-neutral-800 rounded-xl p-4"
            >
        ...
        {[
          ...
          <div
            key={section.title}
            className="bg-[#121214] border border-neutral-800 rounded-2xl p-6"
          >
        ...
        <div className="bg-[#121214] border border-neutral-800 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-neutral-600 mt-1">
              Posted on{" "}
              {new Date(createdAt?.$date || createdAt).toLocaleDateString(
                "en-GB",
              )}
            </p>
          </div>
          <Link href={`/jobs/${id}/apply`} className="w-full sm:w-auto">
            <Button size="md" className="w-full bg-purple-500 hover:bg-purple-600">
              Apply Now
            </Button>
          </Link>
        </div>
